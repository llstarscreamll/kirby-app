...
- feat(desktop): hide print button on ticket printing screen
- refactor(desktop): prevent to always show devtools, only on non packaged targets
- docs: add steps to install global npm packages
- feat: prevent to launch multiple window instances of desktop app
- feat(production): make some validations to require some times employee codes based on authenticated user permissions
- feat(production): make the employee code form field required for users that can create/update logs on behalf other employees
- feat(production): reset employee code form field when creation button is clicked
- feat(production): new ability to create production logs by employee token instead of selecting employee from a dropdown
- feat(employees): new ability to request production registration token generation
