...
export class NoveltyService extends BaseService {
  ...
  public searchNoveltyTypes(query: any = {}): Observable<any> {
    ...
    return this.http.get<any>(endpoint, { headers: this.defaultHeaders, params: query });
  }

  public get(id: string): Observable<any> {
    const endpoint = this.endpoint + id;
    return this.http.get<any>(endpoint, { headers: this.defaultHeaders }).pipe(
  ...
  public update(id: string, noveltyData: any): Observable<any> {
    const endpoint = this.endpoint + id;
    return this.http.put<any>(endpoint, noveltyData, { headers: this.defaultHeaders }).pipe(
      map(response => response.data)
    );
  }
}
