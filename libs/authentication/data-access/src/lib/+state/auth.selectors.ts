...
const getAuthUser = createSelector(
  getAuthState,
  (state: AuthState) => (state.user ? User.fromJson(state.user) : null)
);
const getAuthTokens = createSelector(
  getAuthState,
  (state: AuthState) => state.tokens
);
const getIsLoggedIn = createSelector(
  getAuthState,
  (state: AuthState) => tokensAreValid(state)
);
...
function tokensAreValid(state: AuthState): boolean {
  ...
  const limit = new Date(
    receivedAt.getTime() + secondsToExpire * 6000
  ).getTime();
